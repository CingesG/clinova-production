...
- **Build**: `npm install --include=dev && npx prisma generate && npx prisma db push && npm run build`
- **Start**: `npm run start:prod` (`start:prod` нь `node dist/main.js` — Nest `main.ts`→`dist/main.js`)
- **Анхаар**: `prisma db push` нь migrate-ийн оронд схем синк хийнэ; аль хэвнийг ашиглахыг production төлөвлөгөөнөөсөө сонгоно.
